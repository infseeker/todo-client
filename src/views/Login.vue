  ...
  <user-form>
    ...
    <template v-slot:content>
      <div @keypress.enter="login(username, password)">
        <div class="mb-3">
          <input :placeholder="this.$t('user.usernameEmailPlaceholder')" type="email" v-model.trim="username"
            class="form-control" />
        ...
        <div class="mb-3 form-password-toggle">
          <div class="input-group input-group-merge">
            <input v-if="showPassword" :placeholder="this.$t('user.passwordPlaceholder')" v-model.trim="password"
              class="form-control" />
            <input v-else type="password" :placeholder="this.$t('user.passwordPlaceholder')" v-model.trim="password"
              class="form-control" />
    ...
    <template v-slot:button>
      <button @click="login(username, password)" :disabled="isDisabled" type="button" class="btn btn-primary w-100">{{
          this.$t('user.login')
      }}</button>
    </template>
              ...
              if (this.$user.isAuth) {
                if (this.$user.isAdmin) {
                  ...
                  if (unsavedList && unsavedList.length > 0) {
                    ...
                    this.$router.push({ name: 'lists', params: { unsavedList: true } });
                  } else {
